-- Pulse Oscillator
-- Variable-width pulse with PolyBLEP at both edges. Full ADSR envelope.
...
return {
    ...
    params = {
        ...
        { id="width",   label="Pulse Width",  min=0.05, max=0.95,  default=0.5,  type="float" },
    ...
    new = function(self, args)
        ...
        local width   = self.params[5].default
        local attack  = self.params[6].default
        local decay   = self.params[7].default
        local sustain = self.params[8].default
        local release = self.params[9].default
        ...
        function inst:set_param(id, value)
            ...
            elseif id == "width"   then width   = piper.clamp(value, 0.05, 0.95)
        ...
        function inst:on_message(inlet_id, msg)
            ...
            elseif inlet_id == "pw" and msg.type == "float" then
                width = piper.clamp(msg.v, 0.05, 0.95)
            end
        end
        ...
        function inst:render(out_bufs, n)
            ...
            for i = 0, n - 1 do
                ...
                -- PolyBLEP pulse
                local v = (phase_norm < width) and 1.0 or -1.0
                v = v - polyblep(phase_norm, dt)
                v = v + polyblep(math.fmod(phase_norm - width + 1.0, 1.0), dt)
